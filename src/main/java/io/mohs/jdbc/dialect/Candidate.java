...
public record Candidate(String id, String jobKey, boolean allowConcurrentExecutions, @Nullable String windowName) {
     ...
     * `allow_concurrent_executions`/`window_name`) já são um contrato fixo
     * entre os quatro {@code SELECT}s, então este mapeamento não carrega
     * nada específico de banco.
     */
