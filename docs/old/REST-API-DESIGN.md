...
| `GET /overview/stream` | O mesmo retrato, empurrado por SSE a cada 2s: eventos nomeados `overview`/`jobs`/`nodes`/`runners`/`executions` — polling movido pro servidor, não entrega de eventos |
...
| `POST /executions/{id}/retry` | Retry manual de uma execução `FAILED` (ADR-0033): a MESMA linha rearma como `RETRY_WAITING` devida agora e disputa o claim normal — bypassa a política exaurida. Sem `Idempotency-Key` (retry não deduplica; a idempotência é o CAS). → 202; estado ≠ `FAILED` → 409 |
...
`executionCountsByStatus` carrega só `ENQUEUED`/`RUNNING`/`RETRY_WAITING`
...
`asOf` é o instante do retrato — um só pros 5 eventos do mesmo tick, do
...
desde a ADR-0063, **REVISANDO** a decisão que esta seção trazia desde
2026-08-21 ("`runners` não entra no stream: ocupação de runner é node-local,
e um canal que promete visão de cluster entregaria um número cujo
significado depende de qual nó atendeu o SSE"). O que aquela decisão viu
continua verdadeiro e vira CONTRATO em vez de sumir: `max`/`running` são
**node-local** e respondem pelo nó que atendeu o SSE — atrás de um load
balancer, um nó arbitrário. O frame é publicado porque `overview.RUNNING` é
cluster-wide e, sem denominador, não diz se 59 é folga ou saturação; o
consumidor é obrigado a **rotular o escopo** e NÃO deve dividir um pelo
outro. Capacidade cluster-wide continua não derivável (`/nodes` não carrega
`max`). É leitura em memória do `RunnerRegistry` — zero statement, o mais
barato dos cinco frames. Ver `DASHBOARD-STREAM-REVIEW.md` §5.
