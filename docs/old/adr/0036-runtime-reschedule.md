...
(`RuntimePatchResponse.BOOT_REVERSION_NOTICE`, `../REST-API-DESIGN.md` §"PATCH runtime ×
...
- O operador ganha o lever de emergência sem quebrar "código é a fonte da verdade": o boot
  ...
  PATCH.
- `../REST-API-DESIGN.md` ganha a linha do endpoint; `ScheduleView` passa a ser corpo de
...
do trigger; "agenda alterada sobrescreve"); `../REST-API-DESIGN.md` §"PATCH runtime ×
