...
Companheiro da ADR-0046, no mesmo espírito de `RATE-LIMIT-EVOLUTION.md` e
`BATCH-ARCHITECTURE-REVIEW.md`: a ADR registra o que foi **decidido** — no
...
público documentado em `REST-API-DESIGN.md`, que precisa de decisão do dono
...
**O que é.** Todo assinante recebe todos os frames (quatro à época; cinco desde
a ADR-0063), esteja em que página estiver. Quem está na página de Jobs recebe `executions`; quem está em
...
**Estado (2026-08-29) — a regra acima foi REVOGADA pela ADR-0063 §5.** `runners`
é o quinto frame do stream. O que mudou não foi o custo (ele continua sendo o
frame mais barato dos cinco: leitura em memória do `RunnerRegistry`, sem
consulta) nem o diagnóstico — o número **continua** sendo node-local e continua
dependendo de qual nó atendeu o handshake. O que mudou foi ter aparecido um
consumidor para o qual a alternativa era pior: o tile de concorrência do
Overview precisa do denominador `max`, e sem ele "59 rodando" não distingue
folga de saturação. A conclusão correta não era "não entra no canal" — era
**"entra, e o painel rotula o escopo"**, que é o que a ADR-0063 §6 decidiu e o
`OverviewPage`/`ExecutionActivityChart` implementam, rotulando os dois lados
(`· io @ this node` no tile, `· cluster` na legenda do gráfico).

Consequência operacional que sobrou: `runners` passou a ter **escritor único
enquanto o stream está vivo**. O `refetchInterval` de 2s da página de Runners foi
removido — com os dois, um SSE fixado no nó A e um poll em round-robin
escreveriam contadores de nós diferentes na mesma chave, duas vezes por segundo.
Fora desse estado ainda há três caminhos que refazem a busca contra um nó
arbitrário (a invalidação ao voltar para a aba, o poll de fallback de 15s e o
refresh manual). Eles ficam: trocar de nó uma vez por ação do operador é
legítimo; piscar duas vezes por segundo é ruído.

Isso obrigou uma segunda mudança, e ela vale além de `runners`: o detector de
queda passou a observar **os dados, não o socket**. Um `EventSource` que abriu e
parou de receber bytes (proxy que bufferiza, TCP meio-aberto) nunca dispara
`error` — antes, a página de Runners sobrevivia a isso pelo pulso próprio de 2s,
e sem ele o header afirmaria "live" sobre números congelados. Agora cada frame
recebido rearma o mesmo temporizador que a queda usa. O que o rótulo "live"
afirma passou a ser verdade por construção, para as cinco chaves.
...
## Errata de 2026-08-29 (ADR-0063)

O item 1 diz que a contagem de vazão custa 1,6 ms a 2M linhas. Esse número foi
medido a **1 execução/s** e vinha sendo usado para raciocinar sobre o ponto de
operação de 4.000/s do BASELINE, onde ele não vale. Remedido contra Postgres
18.4 com o DDL de produção de `mohs_attempt`:

| densidade | PT1M | PT10S | as duas |
|---|---:|---:|---:|
| 1 exec/s (2M linhas, 23 dias) | 0,109 ms | 0,093 ms | 0,198 ms |
| 4.000 exec/s (6M linhas, 25 min) | **16,20 ms** | 4,55 ms | 20,99 ms |

Dez vezes mais cara na densidade que importa. E os 1,6 ms originais eram ~98%
overhead de JDBC/conexão: a query custa 0,027 ms de banco naquela forma.

Duas consequências para quem for fazer a medição que o item 1 pede como gatilho
(endpoint sob carga com assinante SSE conectado):

1. **O baseline correto é o de TRÊS counts, não dois** — a ADR-0063 acrescentou
   uma leitura de janela curta ao `/overview`. Por tick ela é grátis (+0,5
   consulta/s, igual com 1 ou 64 assinantes, porque `tick()` compartilha um
   `buildFrames()`); por chamada custa +4,79 ms a 4k/s.
2. **O custo dominante do frame continua sendo o `countActiveByState`** (13,2 ms
   com 500k de backlog) — ~3× a leitura acrescentada. É ali que está o orçamento,
   não na vazão.

Achado correlato, **pré-existente e não corrigido**: `?window=1h`
(`MAX_THROUGHPUT_WINDOW`) vira Parallel Seq Scan quando a janela cobre mais de
~50% do acervo — medido em **394 ms e ~480 MB lidos por requisição** num acervo de
6M linhas com 25 min de retenção. É a combinação retenção curta + tráfego alto,
num endpoint sem autenticação embutida e sem teto de chamadas. **Gatilho:**
primeira instalação com retenção de `mohs_attempt` abaixo de ~2h.
