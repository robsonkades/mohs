...
-- SQL Server: NVARCHAR (DB-5 — VARCHAR não é Unicode), guarda por
-- sys.columns, e EXEC nas etapas que TOCAM a coluna nova: o motor compila
-- o lote inteiro antes de executar, e referenciar coluna adicionada no
-- mesmo lote falha com "Invalid column name".
IF NOT EXISTS (SELECT 1 FROM sys.columns WHERE object_id = OBJECT_ID('mohs_batches', 'U') AND name = 'name')
    ALTER TABLE mohs_batches ADD name NVARCHAR(255);
EXEC('UPDATE mohs_batches SET name = id WHERE name IS NULL');
EXEC('ALTER TABLE mohs_batches ALTER COLUMN name NVARCHAR(255) NOT NULL');
