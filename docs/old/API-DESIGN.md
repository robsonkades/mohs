...
2. **Cadeia pura até o terminal.** Nenhum passo intermediário tem efeito
   ...
   builder. Passos **não-terminais** anotados com `@CheckReturnValue`, mais
   `Mohs#schedule`/`#batch` — cadeia abandonada (o bug clássico do builder
   sem `.build()`) vira warning. Os TERMINAIS ficam de fora (revisto em
   2026-08-29): eles não acrescentam nada a esse objetivo — quem pega
   `mohs.schedule(ref, p);` sem terminal é a anotação nos passos anteriores —
   e cobrariam a linha mais comum da biblioteca,
   `mohs.schedule(ref, payload).now();`, que é um statement correto.
   `Enqueued` é recibo, não resultado a inspecionar.
...
1. **Fronteira por pacote, guardada por ArchUnit:** API pública toda sob
   `io.mohs.core` **[DECIDIDO EM ADR-0015]** (revisa a ADR-0013, que deixava
   fachada/identidade soltas em `io.mohs` raiz):
   - `io.mohs.core` — fachada + identidade compartilhada (`Mohs`, `JobKey`, `JobRef`)
   - `io.mohs.core.schedule` · `io.mohs.core.definition` · `io.mohs.core.execution` ·
     `io.mohs.core.event` · `io.mohs.core.resource` — resto da API pública,
     dividida por concern (ADR-0013)
   ...
   `rest` só enxerga a API pública; `test` não vaza para produção.
