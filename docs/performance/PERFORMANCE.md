...
| `mohs.engine.poll-interval` | 25ms | PISO do loop adaptativo (Phase 6/ADR-G): latência de pickup sob carga | O loop tica neste ritmo enquanto acha trabalho; tick vazio dobra o intervalo até `max-poll-interval`, então poll curto NÃO compra ticks vazios em idle. Subir só se o piso martelar o banco sob carga sustentada — e meça antes |
| `mohs.engine.max-poll-interval` | 2s | TETO do backoff em idle (ADR-G) | Idle custa ~1 lap por teto; o sono real nunca passa de `node-lease-ttl/3` (o heartbeat viaja no tick). Subir o teto além disso não reduz custo — só o cap manda |
