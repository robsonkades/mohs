...
-- um arquivo por dialeto). Prefixo mohs_ em toda tabela — Mohs é biblioteca
...
CREATE TABLE IF NOT EXISTS mohs_job_definitions (
    ...
    next_fire_at    TIMESTAMP,   -- estado do trigger — ver schema-h2.sql
...
-- throughput de claim estável).
-- RETRY_WAITING entrou no predicado do claim — índice parcial só é
...
-- reclaim (lease_expires_at = :expectedLease, anti-ABA) ainda
...
-- por construção; outcome na segunda coluna serve o
-- GROUP BY sem heap fetch (index-only). finished_at cresce monotônico —
-- inserts no fim do índice, mesma localidade do UUIDv7.
