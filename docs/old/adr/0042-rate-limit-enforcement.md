...
**O que o mestre já decidiu.** `../API-DESIGN.md` fixa que rate limit
...
`on-conflict` da ADR-0006. `../MOHS-DOCUMENTO-MESTRE.md` §5 previa "janela
...
11. **Wire.** `RateLimitResponse.currentCount` (stub M2, nunca implementado)
    ...
    agora. Registrado como v0.10 em `../REST-API-DESIGN.md`.
...
- **Contenção — MEDIDA** (Postgres 18.4 em container local, pgbench, sessão de
  ...
  **REVISADO em 2026-08-18** por medição com a rodada real a `batch=1000`:
  ...
  `../RATE-LIMIT-EVOLUTION.md`). Como cada nó
...
## Source
`../API-DESIGN.md` (Recursos nomeados; "Admissão nunca espera capacidade";
semântica cluster-wide do upsert), `../MOHS-DOCUMENTO-MESTRE.md` §5
