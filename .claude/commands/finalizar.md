...
1. List the changed Java files: run `git status --porcelain` and `git diff --name-only HEAD` (include staged and unstaged). If neither `.java` nor persistence files (`.sql`, code under `io.mohs.jdbc`) changed, say so and stop.
