...
backlog. This is the same trade `CLAIM-GRANULARITY.md` analysed for option B,
...
Measured, from `RATE-LIMIT-EVOLUTION.md`: aggregate claim rounds **freeze at
...
`BATCH-ARCHITECTURE-REVIEW.md` documents four defects, two of them *incurable
...
Keep `REST-API-DESIGN.md`'s good decisions (cursor pagination, RFC 7807
...
`ADR-0046` decided *not* to change `/overview/stream` and `DASHBOARD-STREAM-REVIEW.md`
...
Everything else in `API-DESIGN.md` survives verbatim. These four do not:
...
Ten decisions, in the format the brief requires. These become `docs/old/adr/0047`+ in the
existing sequence.
